...
{
    // ”тилита дл€ создани€ структуры протокола
    ...
    public:
        TEST_METHOD(TestProcess_SumTimeForSkype)
        {
            ...
            protocols[1] = build_protocol(14, 0, 0, 16, 0, 0, "Zoom"); // не учитываетс€
            protocols[2] = build_protocol(9, 0, 0, 10, 30, 0, "Skype"); // 5400 секунд (1.5 часа)
            Assert::AreEqual(12600, process(protocols, 3, "Skype")); // ќбщее врем€ 12600 секунд (3.5 часа)
        }
        ...
        TEST_METHOD(TestProcess_ZeroTimeForTeams)
        {
            ...
            Assert::AreEqual(0, process(protocols, 3, "Teams")); // ќбщее врем€ 0 секунд
        }
        ...
        TEST_METHOD(TestProcess_VariousTimesForProgram1)
        {
            ...
            Assert::AreEqual(210, process(protocols, 3, "Program1")); // ќбщее врем€ 210 секунд
        }

        // ƒобавьте свои собственные тесты здесь
    };
}
